...
public:
	...
	/// \brief
	/// advanced output operator for a rational value
	/// \details
	/// This operator<< prints a constructor in the format
	/// [counter/denominator] where both values are printed as
	/// decimal values.
	friend std::ostream & operator<<(std::iostream &lhs, const rational &rhs) {
		return lhs
			<< "["
			<< rhs.counter
			<< "/"
	...
	/// \details
	/// This operator*= multiplies a rational value with a rational value.
	rational &operator*=(const rational &rhs) {
		counter *= rhs.counter;
		denominator *= rhs.denominator;
		reduce();
